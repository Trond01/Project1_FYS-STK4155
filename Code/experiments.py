...
def plot_MSE_R2_experiment(
    ...
    ax[1].plot(
        feature_numbers[1:], R2_train[1:], label="Train", c="r"
    )  # Ignore degree 0 as RD=10^30...
    ...
    if mark_deg_nn:
        ...
        ax[0].plot(
            feature_numbers[xy_deg_indeces], MRS_test[xy_deg_indeces], "o", c="b"
        )
        ax[1].plot(
            feature_numbers[xy_deg_indeces[1:]], R2_test[xy_deg_indeces[1:]], "o", c="b"
        )
...
def plot_beta_experiment(
    ...
    for i in beta_comp_indeces:
        ...
        bi = beta_dict[f"{i}"]["bi"]
        subscript = "".join(
            chr(0x2080 + int(digit)) for digit in str(i)
        )  # Convert 15 to _{15}
...
def analyze_lambda_range(
    ...
    for i in range(lams.shape[0]):
        for j in range(lams.shape[1]):
            fit_data = train_method(
                data, degs[i, j], lams[i, j], plot_or_not=False, test_index=test_index
            )
