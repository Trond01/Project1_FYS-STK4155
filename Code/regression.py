...
from Code.utilities import MSELoss, random_dataset_split_preparation, r2_sampling, R_squared
...
def train_OLS(data, num_features, scale=True, test_index=None, plot_or_not=False, **kwargs):
...
def fit_LASSO(data, lam, plot_or_not=False, max_iter=1000):
    ...
    regressor = linear_model.Lasso(lam, fit_intercept=False, max_iter=max_iter) # We include intercept with False
...
def train_LASSO(data, num_features, lam, scale=True, plot_or_not=False, test_index=None, max_iter=1000):
    data = random_dataset_split_preparation(data, num_features, scale=scale, test_index=test_index)
